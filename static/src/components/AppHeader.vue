<template>
  <header class="fixed z-20 w-full md:relative">
    ...
    <div
      ...
      <div class="ml-auto max-w-xl lg:w-full flex items-center">
        ...
        <span @click="header.showingMobileSearch = true"
              class="cursor-pointer material-symbols-outlined lg:hidden text-4xl font-bold mr-4">
      search
      </span>
      ...
      <div v-if="cart.justAdded"
        ...
        <div class="grid grid-cols-[20%_80%] gap-2 my-2">
          ...
          <div>
            ...
            {{ capitalize(cart.latestItem.listing.type) }} ${{
              (cart.latestItem.listing.price_in_cents / 100).toFixed(2)
            }}
            <a href="/cart">
              <Button
  ...
  <div v-if="header.showingMobileSearch" class="t-20 flex lg:hidden w-full mt-4 absolute z-20 items-center">
    <Search class="w-full"/>
    <div class="mx-4">
      <Button @click="header.showingMobileSearch = false"
              class="!bg-red-200 !text-sm text-gray-500 !px-2 !py-1 !rounded-full" :text="'X'"/>
    </div>
...
import {header} from "../store/header.js";</script>
